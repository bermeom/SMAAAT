...
public class ControlPosition extends AbstractControl implements ActionListener {
    ...
    public ControlPosition(Character character, Vector3f direction) {
        ...
        if (character.getAtractionPoint() == null) {
            if (character.getAllowMovement()) {
                ...
                if ((angle % FastMath.HALF_PI)>0.15) {
                    System.out.println("My Name:"+this.data.getAlias()+" Name: " + spatial.getName());
    ...
    public void move(float tpf) {
        ...
        if (left || right) {
            ...
            walkDirection.addLocal(modelForwardDir.mult(data.getSpeed()));
        }
        if (forward) {
            walkDirection.addLocal(modelForwardDir.mult(data.getSpeed()));
        }
        if (backward) {
            walkDirection.addLocal(modelForwardDir.negate().multLocal(data.getSpeed()));
        }
